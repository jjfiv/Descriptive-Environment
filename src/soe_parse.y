...
struc_ex_cmd:
	...
	| id PERIOD pred  {$$=node(EXPREDALL,$1,$3);}
	| id PERIOD constant 	{$$=node(EXCONS,$1,$3);}
	;
assign_cmd:
...
redfindexpr:
   ...
   |    REDFIND LPAREN id COMMA id COMMA number COMMA number COMMA number COMMA
		number RPAREN
...
applyexpr:
	id LPAREN id RPAREN	{$$=node(APPLY,$1,$3);}
	;
...
cvexpr:	
	NEW VOCAB LBRACE cvarg1 RBRACE	{$$=arg_node(VOCAB, $4);}
	;

csexpr:
	 NEW STRUC LBRACE csargs RBRACE	{$$=arg_node(STRUC, $4);}
	;

cvarg1:
	pred COLON number cvargs	{$$=qnode(CVRELARG, $1, $3, $4);}
	| cvconsarg1			{$$=$1;}
	; 
cvargs:
...
/*	| COMMA constant ASSIGN form csconsdefs
			{$$=qnode(CSCONSDEF, $2, $4, $5);}
*/	;
id:
        CONSTANT                {$$=snode(ID,$1);}
